...
namespace Histogram_Ocen
{
    public partial class GraphWindow : Form
    {
        ...
        {
            Histogram histogram = new Histogram(MainWindow.fileContent); // Wybieranie pliku.
            histogram.GenerateHistogram();
            if (histogram.isEmpty == true) // Sprawdzanie czy plik zawiera wartości liczbowe.
                MessageBox.Show("Plik nie zawiera liczb!");
            else
            {
                chart.Series.Clear(); // Czyścimy serie wykresu.
                Series series = new Series(); // Tworzymy nową serię wykresu.
                ...
                chart.Series.Add(series); // Dodawanie opcji modyfikacji wykresu dla użytkownika.
                chart.ChartAreas["ChartArea1"].AxisX.Title = "Oceny";
                chart.ChartAreas["ChartArea1"].AxisY.Title = "Ilość";
        ...
        private void bt_change_color_Click(object sender, EventArgs e) // Przycisk do zmiany koloru histogramu.
        {
        ...
        public void bt_disable_legend_Click(object sender, EventArgs e) // Przycisk do włączania/wyłączania widoku legendy obok histogramu.
        {
            if (chart.Series["Histogram"].IsVisibleInLegend == false)
            {
                chart.Series["Histogram"].IsVisibleInLegend = true;
                bt_disable_legend.Text = "Schowaj legendę";
            }
            else
            {
                chart.Series["Histogram"].IsVisibleInLegend = false;
                bt_disable_legend.Text = "Wyświetl legendę";
            }
        } 

        private void GraphWindow_Load(object sender, EventArgs e) // Okienko wyboru rodzaju histogramu dla wybranego pliku. Mozliwe rodzaje wykresu; słupkowy, kołowy, punktowy.
        {
        ...
        public void cb_chart_type_SelectedIndexChanged_1(object sender, EventArgs e) // Utwarzanie wykresu innego rodzaju w zależności od wybranej opcji.
        {
        ...
        private void bt_save_to_txt_Click(object sender, EventArgs e) // Zapisywanie do pliku.
        {
            using (SaveFileDialog save_txt = new SaveFileDialog())
            {
                save_txt.Filter = "txt files (*.txt)|*.txt"; // Filtr dla wyboru plików.
                save_txt.RestoreDirectory = true;
                save_txt.FilterIndex = 2;
                save_txt.DefaultExt = "txt";
                
                if (save_txt.ShowDialog() == DialogResult.OK) // Zapis ocen do pliku tekstowego z podziałem na oceny i ich ilość.
                {
